...
sprites['end'].fill('black')
pygame.draw.circle(sprites['end'],'yellow',(10,10),10)
pygame.draw.circle(sprites['end'],'black',(10,10),6)
pygame.draw.rect(sprites['end'],'yellow',pygame.Rect(4,4,12,6))
pygame.draw.circle(sprites['end'],'black',(6,6),3)
pygame.draw.circle(sprites['end'],'black',(14,6),3)
sprites['enemy'].fill('red')
sprites['enemy'].set_colorkey('red')
pygame.draw.circle(sprites['enemy'],'yellow',(10,10),10)
pygame.draw.polygon(sprites['enemy'],'black',((4,14),(8,10),(11,10),(16,14)))
pygame.draw.circle(sprites['enemy'],'black',(6,6),3)
pygame.draw.circle(sprites['enemy'],'black',(14,6),3)
sprites['player'].fill('black')
pygame.draw.rect(sprites['player'],'blue',pygame.Rect(0,0,20,20),5)
...
class CameraGroup(pygame.sprite.Group):
    ...
    def custom_draw(self):
        ...
        if not(player.dead):
            self.a = False
        if player.rect.bottom < 340 or not(self.a):
        ...
        for sprite in self.sprites():
            if (type(sprite) == Enemy) and (player.dead):
                self.surface.blit(sprite.image,sprite.last_topleft+self.offset)
            elif not((type(sprite) == Player) and (player.passed or player.dead)):
...
class End(pygame.sprite.Sprite):
    def __init__(self, *groups,topleft,points):
        global sprites
        super().__init__(*groups)
        self.image = sprites['end']
        self.rect = self.image.get_rect()
        self.rect.topleft = topleft
        self.last_topleft = topleft
        self.timer = 0
        self.points = points
    def update(self, *args, **kwargs):
        ...
        self.timer += dt
        self.rect.centerx = math.cos(self.timer*2)*((self.points[0][0]-self.points[1][0])/2)+((self.points[0][0]+self.points[1][0])/2)
        self.rect.centery = math.cos(self.timer*2)*((self.points[0][1]-self.points[1][1])/2)+((self.points[0][1]+self.points[1][1])/2)
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if event.button == 1 and md == False:
                if self.dead:
                    ...
                    self.dead = False
                    self.rect.centerx = c.x
                    self.rect.centery = c.y
...
for i, level in enumerate(os.listdir(os.path.join(direct,'levels'))):
    ...
    with open(os.path.join(direct,'levels',level),'r') as file:
        ...
        babi.append(End(topleft=walls['end']['pos'],points=walls['end']['points']))
        ...
        if gaming:
            ...
            if player.air and not(player.passed):
                ...
                if not(player.dead):
                    ...
                    texts = [font['12'].render(str(math.degrees(angle))+'°',False,'green'),font['12'].render('x: ??? / Vx: ???',False,'green'),font['12'].render('y: ??? / Vy: ???',False,'green'),font['12'].render('target: x-'+str(end[0].rect.centerx)+' / y-'+str(end[0].rect.centery),False,'green')]
                for i in range(round(150*length)):
