...
class CameraGroup(pygame.sprite.Group):
    ...
    def custom_draw(self):
        ...
        for sprite in self.sprites():
            if not((type(sprite) == Player or type(sprite) == End) and player.dead):
        ...
        if player.rect.bottom > 350:
            ...
            screen.blit(win_text[0],(c.x-win_text[0].get_width()/2,c.y-win_text[0].get_height()))
            screen.blit(win_text[1],(c.x-win_text[1].get_width()/2,c.y-win_text[1].get_height()+20))
...
class Player(pygame.sprite.Sprite):
    ...
    def update(self, *args, **kwargs):
        ...
        if end != [] and self.rect.colliderect(end[0]) and not(self.dead):
            for wall in colliders:
                end[0].kill()
                wall.kill()
            self.dead = True
...
async def main():
    ...
    while running:
        ...
        if gaming:
            ...
            if player.air and not(player.dead):
